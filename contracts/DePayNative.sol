...
/**
 * @title DePayNative Contract
 * @standard WT-DP-1N
 * @description A native currency-based implementation of the DePay system, enabling payments and fee management using native blockchain currency (e.g., Ether).
 * Developed by Wilson Tran.
 */
pragma solidity ^0.8.20;

// Import OpenZeppelin libraries for secure ERC20 interactions
import "@openzeppelin/contracts/token/ERC20/utils/SafeERC20.sol";
import "@openzeppelin/contracts/token/ERC20/IERC20.sol";
import "./interfaces/AggregatorV3Interface.sol"; // Interface for Oracle integration

/**
 * @dev Main contract for the DePayNative payment system.
 */
contract DePayNative {
    // Mapping to store user balances in native currency
    ...
    // Address of the price feed oracle (for native currency price retrieval)
    address public registry_address;

    // Decimals of the price feed oracle
    uint8 public registry_decimals;

    // Owner of the contract (administrative privileges)
     ...
     */
    constructor(address _registry_address) {
    ...
    /**
     * @dev Fallback function to accept native currency deposits.
     * Allows the contract to receive Ether or other native currencies.
     */
     ...
     * @param _fee_percent New fee percentage (must be between 4 and 40 basis points).
     */
    ...
    /**
     * @dev Allows users to withdraw their balance in native currency.
     */
    function withdraw() public {
        ...
        // Transfer the corresponding native currency to the user
    ...
    /**
     * @dev Allows users to make a payment in the native blockchain currency (e.g., ETH).
    ...
    function pay(
        ...
        // Ensure the price is valid and greater than zero
        require(token_price_usd > 0, "Oracle returned invalid price");

        // Calculate the required amount of native currency (e.g., ETH)
        uint256 token_amount_required = (usd_amount * 1e18) / token_price_usd;

        // Ensure the user has sent enough Ether to cover the required amount
        require(msg.value >= token_amount_required, "Insufficient Ether sent");
        ...
        if (msg.value > token_amount_required) {
            uint256 refund = msg.value - token_amount_required;
